export const LIGHT_GREEN = "#009E60";
export const DARK_GREEN = "DarkGreen";
export const LIGHT_BLUE = "#6495ED";
export const DARK_BLUE = "Blue";
export const LIGHT_YELLOW = "#FFAA33";
export const DARK_YELLOW = "Goldenrod";
export const LIGHT_BROWN = "#954535";
export const DARK_BROWN = "#800020";
export const LIGHT_GREY = "Grey";
export const DARK_GREY = "DarkGrey";

export const BLACK = "Black";
export const RED = "Red";
export const PURPLE = "Purple";
export const ORANGE_BROWN = "#967969";
