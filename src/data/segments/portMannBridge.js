import { UNITED_PORT_MANN } from "../intersections";
import { ROUTES } from "../routes";
import { ROUTE_VIDEOS } from "../videos/routes";

const BRIDGE_END = [49.20605, -122.80104];
const BRIDGE_END_112_AVE = [49.20605, -122.80081];
const FRASER_HEIGHTS_110A_W = [49.20335, -122.79543];

const ELEVATION_START = [49.22731, -122.82029];
const ELEVATION_END = [49.21129, -122.80828];

const EAST_OVERPASS_START = [49.20601, -122.80307];
const EAST_OVERPASS_END = [49.206, -122.80183];

export const PORT_MANN_BRIDGE = [
  // east connections
  {
    description: "east overpass: west",
    type: "mixed",
    elevation: 0.5,
    videoIds: [ROUTE_VIDEOS.portMannWB.id],
    videoIdsStartAtStart: [ROUTE_VIDEOS.portMannWB.id],
    positions: [
      [49.20608, -122.80446],
      [49.20615, -122.80436],
      [49.20618, -122.80424],
      [49.20618, -122.80376],
      [49.20613, -122.80362],
      [49.20603, -122.80357],
      EAST_OVERPASS_START,
    ],
  },
  {
    description: "east overpass",
    type: "mixed",
    elevation: 1,
    videoIds: [ROUTE_VIDEOS.portMannWB.id],
    positions: [EAST_OVERPASS_START, EAST_OVERPASS_END],
  },
  {
    description: "east overpass: east",
    type: "mixed",
    elevation: 0.5,
    videoIds: [ROUTE_VIDEOS.portMannWB.id],
    positions: [
      EAST_OVERPASS_END,
      [49.206, -122.80163],
      [49.20594, -122.80153],
      [49.20584, -122.80144],
      [49.2058, -122.80135],
      [49.2058, -122.80121],
      [49.20583, -122.80111],
      [49.2059, -122.80104],
      BRIDGE_END,
    ],
  },
  {
    description: "to 152",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.portMannEB.id],
    positions: [BRIDGE_END, BRIDGE_END_112_AVE],
  },
  {
    description: "152, 111a, 153a, 110a",
    type: "quiet",
    videoIds: [ROUTE_VIDEOS.portMannEB.id],
    positions: [
      BRIDGE_END_112_AVE,
      [49.20522, -122.80081],
      [49.20517, -122.80078],
      [49.20514, -122.80068],
      [49.20515, -122.79737],
      [49.20513, -122.79713],
      [49.20508, -122.79694],
      [49.20502, -122.79683],
      [49.20494, -122.79672],
      [49.20485, -122.79665],
      [49.20471, -122.79659],
      [49.20425, -122.79659],
      [49.20389, -122.79662],
      [49.20371, -122.79659],
      [49.2036, -122.79651],
      [49.20351, -122.7964],
      [49.2034, -122.79619],
      [49.20336, -122.79598],
      FRASER_HEIGHTS_110A_W,
    ],
  },
  {
    description: "fraser heights greenway (partial)",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.portMannEB.id],
    videoIdsEndAtEnd: [ROUTE_VIDEOS.portMannEB.id],
    positions: [FRASER_HEIGHTS_110A_W, [49.20245, -122.79544]],
  },

  // bridge
  {
    routeNames: [ROUTES.portMannBridge.name],
    description: "west end",
    type: "mixed",
    elevation: 0.5,
    videoIds: [ROUTE_VIDEOS.portMannEB.id, ROUTE_VIDEOS.portMannWB.id],
    positions: [
      UNITED_PORT_MANN,
      [49.22783, -122.82023],
      [49.22784, -122.82051],
      [49.22788, -122.82083],
      [49.22805, -122.82122],
      [49.22809, -122.82137],
      [49.2281, -122.8218],
      [49.22805, -122.82194],
      [49.22801, -122.82197],
      [49.22795, -122.82196],
      [49.22791, -122.82194],
      [49.22787, -122.82186],
      ELEVATION_START,
    ],
  },
  {
    routeNames: [ROUTES.portMannBridge.name],
    type: "mixed",
    description: "primary",
    elevation: 1,
    videoIds: [ROUTE_VIDEOS.portMannEB.id, ROUTE_VIDEOS.portMannWB.id],
    positions: [
      ELEVATION_START,
      [49.22699, -122.81938],
      [49.22678, -122.81885],
      [49.2266, -122.81844],
      [49.22645, -122.81812],
      [49.22617, -122.81759],
      [49.22581, -122.81696],
      [49.22537, -122.81631],
      [49.22492, -122.81577],
      [49.22456, -122.81539],
      [49.22418, -122.81504],
      [49.22383, -122.81476],
      [49.22348, -122.81451],
      [49.2231, -122.81425],
      [49.22261, -122.81398],
      [49.22198, -122.81369],
      [49.22155, -122.8135],
      [49.22146, -122.81342],
      [49.22019, -122.81287],
      [49.21938, -122.81252],
      [49.21815, -122.81202],
      [49.21736, -122.8117],
      [49.21666, -122.81142],
      [49.21565, -122.81101],
      [49.21515, -122.81082],
      [49.21473, -122.81066],
      [49.21431, -122.81051],
      [49.21405, -122.81041],
      [49.21395, -122.8104],
      [49.21324, -122.81008],
      [49.21289, -122.80988],
      [49.21239, -122.8095],
      [49.21198, -122.80911],
      [49.21168, -122.80877],
      ELEVATION_END,
    ],
  },
  {
    routeNames: [ROUTES.portMannBridge.name],
    description: "east end",
    type: "mixed",
    elevation: 0.5,
    videoIds: [ROUTE_VIDEOS.portMannWB.id, ROUTE_VIDEOS.portMannEB.id],
    positions: [
      ELEVATION_END,
      [49.21095, -122.80778],
      [49.21054, -122.80722],
      [49.2102, -122.8067],
      [49.21004, -122.80639],
      [49.20987, -122.80624],
      [49.20932, -122.80554],
      [49.20801, -122.80404],
      [49.20663, -122.80253],
      [49.20616, -122.80202],
      [49.20611, -122.80191],
      [49.20607, -122.80163],
      BRIDGE_END,
    ],
  },
];
