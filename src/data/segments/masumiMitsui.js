import {
  ANGUS_59TH,
  BORDEN_59TH,
  CAMBIE_58TH_E,
  CAMBIE_59TH_NE_E,
  CAMBIE_59TH_NW,
  HEATHER_59TH,
  INVERNESS_59TH,
  ONTARIO_58TH,
  ONTARIO_59TH,
  PRINCE_EDWARD_59TH,
  VIVIAN_57TH,
  VIVIAN_LYNBROOK,
} from "../intersections";
import { ROUTES } from "../routes";
import { ROUTE_VIDEOS } from "../videos/routes";

const GRANVILLE_59TH_W = [49.21748, -123.14063];
const GRANVILLE_LANE_E_59TH = [49.21736, -123.13943];
const CAMBIE_58TH_EE = [49.21784, -123.1166];
const ALBERTA_58TH = [49.21774, -123.1133];
const PAULSON_59TH_W = [49.21697, -123.11867];
const LEFT_PAINTED_BEGIN = [49.21697, -123.11829];
const ARGYLE_59TH_E = [49.21629, -123.07191];
const NASSAU_PRESTWICK = [49.21735, -123.07125];
const ELLIOTT_57TH = [49.21799, -123.05422];
const ELLIOTT_ASHBURN = [49.21855, -123.05467];
const ASHBURN_UPLAND = [49.21764, -123.05581];

export const MASUMI_MITSUI_GREENWAY = [
  {
    description: "connect cambie on 58th",
    type: "quiet",
    positions: [CAMBIE_58TH_E, CAMBIE_58TH_EE],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "angus to granville",
    type: "quiet",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    videoIdsStartAtStart: [ROUTE_VIDEOS.masumiMitsuiEb.id],
    videoIdsEndAtStart: [ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      ANGUS_59TH,
      [49.21768, -123.14384],
      [49.21765, -123.14143],
      [49.21763, -123.14122],
      [49.21761, -123.14109],
      [49.21757, -123.1409],
      GRANVILLE_59TH_W,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "crossing granville",
    type: "dedicated",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      GRANVILLE_59TH_W,
      [49.21746, -123.14029],
      [49.21744, -123.13997],
      [49.21736, -123.13972],
      [49.21735, -123.13965],
      [49.21738, -123.13955],
      [49.21738, -123.13951],
      GRANVILLE_LANE_E_59TH,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "granville to ontario",
    type: "quiet",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      GRANVILLE_LANE_E_59TH,
      [49.21738, -123.13933],
      [49.21739, -123.13926],
      [49.21735, -123.13778],
      [49.2173, -123.13544],
      [49.21725, -123.13406],
      [49.21717, -123.13133],
      [49.21712, -123.12933],
      [49.21706, -123.12719],
      [49.21703, -123.12457],
      HEATHER_59TH,
      [49.21699, -123.12007],
      PAULSON_59TH_W,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "eb: paulson to...",
    type: "quiet",
    oneWay: "recommended",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id],
    positions: [PAULSON_59TH_W, LEFT_PAINTED_BEGIN],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "eb: ...to cambie",
    type: "painted",
    oneWay: "required",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id],
    positions: [
      LEFT_PAINTED_BEGIN,
      [49.21698, -123.11822],
      [49.21697, -123.11718],
      CAMBIE_59TH_NW,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "wb: cambie to paulson",
    type: "dedicated",
    oneWay: "required",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [CAMBIE_59TH_NW, [49.21701, -123.11858], PAULSON_59TH_W],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "along cambie",
    type: "combined",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [CAMBIE_59TH_NW, CAMBIE_59TH_NE_E, CAMBIE_58TH_EE],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "cambie to alberta",
    type: "quiet",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      CAMBIE_58TH_EE,
      [49.2178, -123.11343],
      [49.21778, -123.11337],
      ALBERTA_58TH,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "alberta to ontario",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      ALBERTA_58TH,
      [49.21776, -123.1127],
      [49.21774, -123.11236],
      [49.21775, -123.11158],
      [49.2177, -123.11104],
      [49.21769, -123.10969],
      [49.21766, -123.10759],
      [49.21766, -123.1069],
      [49.21767, -123.10662],
      [49.21768, -123.10651],
      [49.21771, -123.10636],
      ONTARIO_58TH,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "ontario to argyle",
    type: "quiet",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      ONTARIO_59TH,
      [49.21676, -123.10195],
      PRINCE_EDWARD_59TH,
      [49.21654, -123.09099],
      [49.21657, -123.09084],
      [49.21648, -123.08549],
      [49.21642, -123.08162],
      [49.21641, -123.08082],
      INVERNESS_59TH,
      [49.2164, -123.07705],
      [49.21639, -123.0746],
      BORDEN_59TH,
      ARGYLE_59TH_E,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "along argyle",
    type: "dedicated",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      ARGYLE_59TH_E,
      [49.21678, -123.07181],
      [49.21716, -123.07164],
      [49.21739, -123.07149],
      NASSAU_PRESTWICK,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "argyle to split",
    type: "quiet",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id, ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      NASSAU_PRESTWICK,
      [49.21707, -123.07025],
      [49.21749, -123.06981],
      [49.21775, -123.06931],
      [49.21789, -123.06892],
      [49.21803, -123.06833],
      [49.21808, -123.0678],
      [49.21807, -123.06591],
      [49.21802, -123.06331],
      [49.21799, -123.06074],
      [49.21797, -123.05783],
      [49.21797, -123.05696],
      [49.21788, -123.05651],
      [49.21778, -123.05613],
      ASHBURN_UPLAND,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "eastbound branch",
    type: "quiet",
    oneWay: "recommended",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiEb.id],
    videoIdsEndAtEnd: [ROUTE_VIDEOS.masumiMitsuiEb.id],
    positions: [
      ASHBURN_UPLAND,
      [49.21734, -123.05541],
      [49.21711, -123.05522],
      [49.21696, -123.05514],
      [49.21709, -123.05446],
      [49.21715, -123.05374],
      [49.2171, -123.05201],
      VIVIAN_LYNBROOK,
    ],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "westbound branch 57th",
    type: "quiet",
    oneWay: "recommended",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiWb.id],
    videoIdsStartAtStart: [ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [VIVIAN_57TH, [49.21806, -123.05126], ELLIOTT_57TH],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "westbound branch elliott path",
    type: "dedicated",
    oneWay: "required",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [ELLIOTT_57TH, [49.21841, -123.05461], ELLIOTT_ASHBURN],
  },
  {
    routeNames: [ROUTES.masumiMitsui.name],
    description: "westbound branch ashburn",
    type: "quiet",
    oneWay: "recommended",
    videoIds: [ROUTE_VIDEOS.masumiMitsuiWb.id],
    positions: [
      ELLIOTT_ASHBURN,
      [49.21839, -123.055],
      [49.21815, -123.05518],
      [49.21793, -123.05541],
      ASHBURN_UPLAND,
    ],
  },
];
