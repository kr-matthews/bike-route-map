import {
  AIRPORT_TEMPLETON_STATION_SW,
  AURORA_MILLER_SE,
  AURORA_MILLER_SW,
  CESSNA_MILLER_E,
  GRANT_MCCONACHIE_WB_START,
  NO_2_BRIDGE_SIDEWALK_NE,
  NO_2_RUSS_BAKER_E,
  NO_2_RUSS_BAKER_W,
  POST_SEA_ISLAND_CONNECTOR,
  RUSS_BAKER_INGLIS_NW,
  RUSS_BAKER_INGLIS_SW,
  SEA_ISLAND_CONNECTOR_ARTHUR_LAING_N,
  SEA_ISLAND_PATH_GILBERT_N,
  SEA_ISLAND_WAY_POST_BUS_CONNECTION,
  TEMPLETON_MILLER_SE,
  TEMPLETON_TEMPLETON_STATION_SE_E,
  TEMPLETON_TEMPLETON_STATION_SE_SE,
} from "../intersections";
import { ROUTES } from "../routes";
import { ROUTE_VIDEOS } from "../videos/routes";

const TEMPLETON_TERRACE_NE = [49.19034, -123.15052];
const SEA_ISLAND_PATH_PRE_NO_2_W = [49.17875, -123.1551];
const LARRY_BERG_PARK_SPLIT = [49.1847, -123.15035];
const MILLER_PUMP_SPLIT = [49.19219, -123.1393];
const UNDERNEATH_BRIDGES = [49.19241, -123.13883];
const AIRPORT_S_END = [49.19373, -123.13903];
const PRE_TEMPLETON_STATION_SPLIT = [49.19565, -123.13979];
const SPLIT_TO_SEA_ISLAND_WAY = [49.19234, -123.14002];
const POST_ARTHUR_LAING_SPLIT = [49.19548, -123.14004];
const TEMPLETON_TEMPLETON_SE = [49.19738, -123.14478];

export const SEA_ISLAND_PATH = [
  {
    description: "to sea island way",
    type: "mixed",
    oneWay: "required",
    positions: [
      SPLIT_TO_SEA_ISLAND_WAY,
      [49.19238, -123.1398],
      SEA_ISLAND_WAY_POST_BUS_CONNECTION,
    ],
  },
  {
    description: "miller connection 2",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.millerWb.id],
    positions: [
      CESSNA_MILLER_E,
      [49.19232, -123.14021],
      SPLIT_TO_SEA_ISLAND_WAY,
      [49.19235, -123.1396],
      [49.19232, -123.13936],
      MILLER_PUMP_SPLIT,
    ],
  },
  {
    description: "arthur laing sb to first split",
    type: "dedicated",
    videoIds: [
      ROUTE_VIDEOS.grantMcConachieWb.id,
      ROUTE_VIDEOS.arthurLaingSb.id,
    ],
    videoIdsEndAtEnd: [ROUTE_VIDEOS.arthurLaingSb.id],
    positions: [
      SEA_ISLAND_CONNECTOR_ARTHUR_LAING_N,
      [49.19527, -123.13996],
      POST_ARTHUR_LAING_SPLIT,
    ],
  },
  {
    description: "arthur laing sb first split to second split",
    type: "dedicated",
    positions: [POST_ARTHUR_LAING_SPLIT, PRE_TEMPLETON_STATION_SPLIT],
  },
  {
    description: "arthur laing sb first split to grant mcconachie",
    type: "dedicated",
    oneWay: "recommended",
    videoIds: [ROUTE_VIDEOS.grantMcConachieWb.id],
    positions: [
      POST_ARTHUR_LAING_SPLIT,
      [49.19572, -123.14033],
      [49.19586, -123.14055],
      [49.19598, -123.14076],
      [49.19612, -123.14121],
      [49.19617, -123.1414],
      [49.1962, -123.14172],
      [49.19622, -123.14213],
      [49.19621, -123.1422],
      [49.19614, -123.14236],
      GRANT_MCCONACHIE_WB_START,
    ],
  },
  {
    description: "nb: no. 2 sidewalk to russ baker",
    type: "dedicated",
    positions: [
      NO_2_RUSS_BAKER_E,
      [49.17832, -123.15504],
      NO_2_BRIDGE_SIDEWALK_NE,
    ],
  },
  {
    description: "sb: path to russ baker",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.no2BridgeSb.id],
    videoIdsStartAtStart: [ROUTE_VIDEOS.no2BridgeSb.id],
    positions: [
      SEA_ISLAND_PATH_PRE_NO_2_W,
      [49.17872, -123.15506],
      NO_2_RUSS_BAKER_W,
    ],
  },

  // sea island center branch
  {
    description: "connecting to sea island center",
    type: "other",
    undesignated: true,
    videoIds: [ROUTE_VIDEOS.seaIslandPathSeaIslandCenterNb.id],
    videoIdsEndAtEnd: [ROUTE_VIDEOS.seaIslandPathSeaIslandCenterNb.id],
    positions: [
      AURORA_MILLER_SW,
      [49.19221, -123.15717],
      [49.1923, -123.15729],
      [49.19228, -123.15738],
      [49.19227, -123.15747],
      [49.19227, -123.15774],
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "sea island center station connection",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathSeaIslandCenterNb.id],
    positions: [
      AURORA_MILLER_SE,
      [49.19193, -123.15671],
      [49.1919, -123.15658],
      [49.1919, -123.15443],
      [49.19192, -123.15434],
      [49.19196, -123.1543],
      [49.19205, -123.15425],
      [49.19205, -123.15396],
      [49.19044, -123.15398],
      [49.19036, -123.15394],
      TEMPLETON_TERRACE_NE,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "templeton st path, park to terrace",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathSeaIslandCenterNb.id],
    positions: [
      LARRY_BERG_PARK_SPLIT,
      [49.18485, -123.15035],
      [49.18492, -123.15033],
      [49.18498, -123.15042],
      [49.18504, -123.15044],
      [49.18575, -123.15041],
      [49.18586, -123.15045],
      [49.18599, -123.15053],
      [49.18606, -123.15055],
      [49.18732, -123.15053],
      [49.18753, -123.15056],
      [49.19022, -123.15051],
      TEMPLETON_TERRACE_NE,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "templeton last block",
    type: "mixed",
    positions: [
      TEMPLETON_TERRACE_NE,
      [49.19044, -123.15052],
      [49.19167, -123.15049],
      [49.19193, -123.15056],
      TEMPLETON_MILLER_SE,
    ],
  },

  // main path
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "under no. 2 bridge",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.no2BridgeNb.id, ROUTE_VIDEOS.seaIslandPathNb.id],
    videoIdsEndAtEnd: [ROUTE_VIDEOS.no2BridgeNb.id],
    positions: [
      NO_2_BRIDGE_SIDEWALK_NE,
      [49.17826, -123.15502],
      [49.17819, -123.15492],
      [49.17801, -123.15534],
      [49.17803, -123.15552],
      [49.17807, -123.15572],
      [49.17804, -123.15615],
      [49.17809, -123.15615],
      [49.17813, -123.15614],
      [49.17819, -123.15602],
      [49.1783, -123.15574],
      [49.17839, -123.15559],
      [49.17848, -123.15547],
      [49.17857, -123.15539],
      [49.17872, -123.15517],
      SEA_ISLAND_PATH_PRE_NO_2_W,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "no. 2 to inglis",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id],
    positions: [
      SEA_ISLAND_PATH_PRE_NO_2_W,
      [49.17879, -123.15508],
      [49.17901, -123.15487],
      [49.1795, -123.15428],
      [49.17965, -123.15416],
      [49.17972, -123.15407],
      RUSS_BAKER_INGLIS_SW,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "crossing inglis",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id, ROUTE_VIDEOS.inglisEb.id],
    videoIdsEndAtEnd: [ROUTE_VIDEOS.inglisEb.id],
    positions: [
      RUSS_BAKER_INGLIS_SW,
      [49.17979, -123.1538],
      [49.17996, -123.15358],
      RUSS_BAKER_INGLIS_NW,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "inglis to gilbert",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id],
    positions: [
      RUSS_BAKER_INGLIS_NW,
      [49.18015, -123.15357],
      [49.18018, -123.15347],
      [49.18078, -123.15267],
      SEA_ISLAND_PATH_GILBERT_N,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "gilbert to larry berg flight path park",
    type: "mixed",
    videoIds: [
      ROUTE_VIDEOS.seaIslandPathSeaIslandCenterNb.id,
      ROUTE_VIDEOS.seaIslandPathNb.id,
    ],
    videoIdsStartAtStart: [ROUTE_VIDEOS.seaIslandPathSeaIslandCenterNb.id],
    positions: [
      SEA_ISLAND_PATH_GILBERT_N,
      [49.18177, -123.15145],
      [49.18177, -123.15145],
      [49.18254, -123.15042],
      [49.18269, -123.15028],
      [49.18279, -123.1502],
      [49.18297, -123.15013],
      [49.18315, -123.15011],
      [49.18346, -123.15017],
      [49.18388, -123.15036],
      [49.18393, -123.15037],
      [49.18398, -123.15036],
      [49.1841, -123.15029],
      [49.18416, -123.15027],
      [49.18426, -123.15028],
      [49.18456, -123.15034],
      LARRY_BERG_PARK_SPLIT,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "larry berg flight path park to miller pump",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id],
    positions: [
      LARRY_BERG_PARK_SPLIT,
      [49.18472, -123.15018],
      [49.18472, -123.14969],
      [49.18469, -123.14952],
      [49.18469, -123.14938],
      [49.18472, -123.14923],
      [49.18476, -123.14908],
      [49.18478, -123.14893],
      [49.18477, -123.14878],
      [49.1847, -123.14796],
      [49.1847, -123.14781],
      [49.18472, -123.14764],
      [49.18476, -123.1475],
      [49.18484, -123.14732],
      [49.1859, -123.14599],
      [49.18594, -123.14589],
      [49.18599, -123.14567],
      [49.18603, -123.1456],
      [49.18629, -123.14534],
      [49.18632, -123.14528],
      [49.18612, -123.14466],
      [49.1861, -123.14453],
      [49.18612, -123.14446],
      [49.18621, -123.14423],
      [49.18625, -123.14415],
      [49.18631, -123.14411],
      [49.18645, -123.14407],
      [49.18653, -123.14407],
      [49.18661, -123.14411],
      [49.18671, -123.14416],
      [49.18686, -123.14421],
      [49.18706, -123.14416],
      [49.18716, -123.14412],
      [49.18732, -123.14402],
      [49.18743, -123.14396],
      [49.18757, -123.14388],
      [49.18768, -123.14379],
      [49.18785, -123.14356],
      [49.18791, -123.14351],
      [49.18785, -123.1432],
      [49.18784, -123.1431],
      [49.18865, -123.14205],
      [49.18883, -123.14166],
      [49.1891, -123.14124],
      [49.18934, -123.14097],
      [49.18954, -123.14081],
      [49.1897, -123.14064],
      [49.18983, -123.14053],
      [49.1901, -123.14026],
      [49.19054, -123.13995],
      [49.19072, -123.13981],
      [49.191, -123.13965],
      [49.19141, -123.13941],
      [49.19184, -123.13911],
      [49.19199, -123.1391],
      [49.19208, -123.13907],
      [49.19214, -123.13927],
      [49.19214, -123.13936],
      MILLER_PUMP_SPLIT,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "miller pump to under bridges",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id, ROUTE_VIDEOS.millerWb.id],
    videoIdsStartAtEnd: [ROUTE_VIDEOS.millerWb.id],
    positions: [
      MILLER_PUMP_SPLIT,
      [49.19218, -123.13912],
      [49.19221, -123.13899],
      [49.19225, -123.13893],
      [49.19234, -123.13886],
      UNDERNEATH_BRIDGES,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "under bridges to airport rd",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id],
    positions: [
      UNDERNEATH_BRIDGES,
      [49.19253, -123.13871],
      [49.19274, -123.1386],
      [49.19283, -123.13857],
      [49.19294, -123.1386],
      [49.19301, -123.13869],
      [49.19305, -123.13881],
      [49.19319, -123.13929],
      [49.1932, -123.13937],
      [49.19319, -123.13944],
      [49.19318, -123.13958],
      [49.19317, -123.13965],
      [49.19318, -123.13972],
      [49.19323, -123.13986],
      POST_SEA_ISLAND_CONNECTOR,
      [49.19352, -123.13956],
      [49.19366, -123.13923],
      AIRPORT_S_END,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "airport rd",
    type: "quiet",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id],
    positions: [
      AIRPORT_S_END,
      [49.19519, -123.13803],
      [49.19529, -123.13794],
      [49.19536, -123.13784],
      [49.19552, -123.13755],
      [49.19561, -123.13741],
      [49.19575, -123.13727],
      AIRPORT_TEMPLETON_STATION_SW,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "airport rd to templeton station",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id],
    positions: [
      AIRPORT_TEMPLETON_STATION_SW,
      [49.1961, -123.13725],
      [49.19617, -123.13753],
      [49.19624, -123.13781],
      [49.19626, -123.13811],
      [49.19624, -123.13825],
      [49.19617, -123.13833],
      [49.19604, -123.1384],
      [49.19589, -123.13848],
      [49.19579, -123.13859],
      [49.19559, -123.13889],
      [49.19554, -123.13902],
      [49.19551, -123.13917],
      [49.1955, -123.13931],
      [49.19551, -123.13945],
      [49.19556, -123.13959],
      PRE_TEMPLETON_STATION_SPLIT,
      [49.1961, -123.14058],
      [49.19622, -123.1407],
      [49.19645, -123.14076],
      [49.19657, -123.14164],
      [49.19666, -123.14213],
      [49.19678, -123.1431],
      [49.19685, -123.14346],
      [49.19693, -123.14376],
      [49.19703, -123.14405],
      [49.19727, -123.14453],
      TEMPLETON_TEMPLETON_SE,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "templeton station to templeton st",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.nServiceEb.id],
    videoIdsEndAtStart: [ROUTE_VIDEOS.nServiceEb.id],
    positions: [
      TEMPLETON_TEMPLETON_SE,
      [49.19742, -123.14495],
      [49.19745, -123.14511],
      [49.19745, -123.14519],
      [49.19744, -123.14531],
      [49.19746, -123.14555],
      [49.19743, -123.14613],
      [49.1974, -123.14685],
      [49.19734, -123.14724],
      [49.19725, -123.1476],
      [49.19692, -123.14839],
      [49.19685, -123.14855],
      [49.19682, -123.14863],
      [49.19677, -123.1487],
      [49.19669, -123.14876],
      [49.19665, -123.14904],
      [49.19665, -123.14919],
      [49.19668, -123.14934],
      [49.19667, -123.1496],
      TEMPLETON_TEMPLETON_STATION_SE_E,
    ],
  },
  {
    routeNames: [ROUTES.seaIslandPath.name],
    description: "little bit at templeton st",
    type: "mixed",
    positions: [
      TEMPLETON_TEMPLETON_STATION_SE_E,
      TEMPLETON_TEMPLETON_STATION_SE_SE,
    ],
  },
  {
    description: "templeton station",
    type: "mixed",
    videoIds: [ROUTE_VIDEOS.seaIslandPathNb.id],
    videoIdsEndAtEnd: [ROUTE_VIDEOS.seaIslandPathNb.id],
    positions: [
      TEMPLETON_TEMPLETON_SE,
      [49.19725, -123.14486],
      [49.19718, -123.14495],
      [49.19714, -123.14504],
      [49.19713, -123.1451],
      [49.19709, -123.14557],
      [49.19704, -123.14589], // station
    ],
  },
];
