...
function BackgroundMapSetting() {
  const { tileLayerKey, setTileLayerKey } = useContext(SettingContext);
...
function DefaultSpeedSetting() {
  const { defaultSpeed, setDefaultSpeed } = useContext(SettingContext);
    ...
    <div style={{ padding: "4px" }}>
      ...
      {defaultSpeed === 4 && (
        <div style={{ paddingTop: 5 }}>
          <em>
            Use the YouTube built-in controls to 2x speed, for an overall effect
            of 8x.
          </em>
        </div>
      )}
